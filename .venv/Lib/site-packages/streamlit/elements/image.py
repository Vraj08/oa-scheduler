    ...
    def image(
        ...
        *,
        ...
        width : bool
            ...
            container. If ``width`` is ``False`` (default),
            Streamlit sets the image's width according to ``width``. If
            ``width`` is ``True``, Streamlit sets the width of
            the image to match the width of the parent container.

        .. deprecated::
            ``width`` is deprecated and will be removed in a
        ...
        if use_column_width is not None:
            if width is not None:
                raise StreamlitAPIException(
                    "`width` and `use_column_width` cannot be set at the same time.",
                    "Please utilize `width` since `use_column_width` is deprecated.",
                )
